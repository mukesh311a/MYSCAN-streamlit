...
.topbar { position: sticky; top: 0; z-index: 1000; background: rgba(255,255,255,.95); backdrop-filter: blur(8px);
  border-bottom: 1px solid #e6eef6; margin-bottom: 18px; padding: 10px 14px; border-radius: 0 0 16px 16px; }
...
.card { background: #ffffff; border:1px solid #e6eef6; border-radius: 16px; padding: 22px;
  box-shadow: 0 12px 30px rgba(12, 35, 64, .06); color: #0b1b3a; }
...
.stTextInput>div>div>input, .stTextInput>label, .stTextArea textarea {
  background:#fff!important; color:#0b1b3a!important; border:1px solid #e6eef6!important; border-radius:10px!important;
}

/* Buttons: orange primary, blue secondary, ghost */
.stButton>button, .stLinkButton>button {
  ...
  background:linear-gradient(180deg,#ffe6cc,#ffcc99)!important;
  box-shadow:0 8px 20px rgba(255,153,51,.20)!important;
}
...
.row .stButton, .row .stLinkButton { margin-right:8px; }
</style>
...
if page == "admin":
    ...
    with c3:
        ...
        for r in rows:
            ...
            col = st.container()
            with col:
                # row layout with enough width to avoid wrapping
                ...
                with d:
                    if r['is_active']:
                        if st.button("Deactivate", key=f"btn-deact-{r['id']}"):
                            set_active(r['id'], False); st.rerun()
                    else:
                        if st.button("Reactivate", key=f"btn-react-{r['id']}"):
                            set_active(r['id'], True); st.rerun()
                with e:
                    if st.button("Regenerate", key=f"regen-{r['id']}"):
...
if page == "user":
    ...
    with c2:
        ccid = st.text_input("Car ID"); csec = st.text_input("Owner Secret")
    ...
    with c3:
        ...
        for car in mycars:
            ...
            with st.form(f"edit-{car['id']}"):
                ...
                save = cols[0].form_submit_button("Save changes")
                if car["is_active"]:
                    deact = cols[1].form_submit_button("Deactivate QR")
                    react = None
                else:
                    react = cols[1].form_submit_button("Reactivate QR")
                    deact = None
            if save:
